...
class StorageService extends GetxService {
  ...
  Future<void> saveThemeMode(bool isDarkMode) => _box.write('darkMode', isDarkMode);

  // Language
  String getLanguage() => _box.read('language') ?? 'en';
  Future<void> saveLanguage(String language) => _box.write('language', language);

  // Onboarding
  bool getOnboardingStatus() => _box.read('onboardingComplete') ?? false;
  Future<void> saveOnboardingStatus(bool status) => _box.write('onboardingComplete', status);
  
  // Location Permission
  bool getLocationPermissionStatus() => _box.read('locationPermissionGranted') ?? false;
  Future<void> saveLocationPermissionStatus(bool status) => _box.write('locationPermissionGranted', status);
  
  // Notification Permission
  bool getNotificationPermissionStatus() => _box.read('notificationPermissionGranted') ?? false;
  Future<void> saveNotificationPermissionStatus(bool status) => _box.write('notificationPermissionGranted', status);
  ...
  Future<void> saveUserData(Map<String, dynamic> userData) => _box.write('userData', userData);
  Future<void> clearUserData() => _box.remove('userData');
  
  // Medplum Access Token
  String? getAccessToken() => _box.read('medplum_access_token');
  Future<void> saveAccessToken(String token) => _box.write('medplum_access_token', token);
  ...
  Future<void> saveLoginSession(Map<String, dynamic> session) => _box.write('medplum_login_session', session);
  ...
  Future<void> saveOrganizationId(String organizationId) => _box.write('organizationId', organizationId);
